...
export default function SharedLinksPage() {
  ...
  const visibleLinks = useMemo(() => {
    ...
    return term
      ? links.filter((link) =>
          `${titleFor(link)} ${link.url}`.toLowerCase().includes(term),
        )
      : links;
  }, [links, query]);
  ...
  const update = async (link: SharedLink, changes: Record<string, unknown>) => {
    try {
      ...
      setLinks((current) =>
        current.map((item) =>
          item._id === link._id ? { ...item, ...data.shareLink } : item,
        ),
      );
      return true;
    } catch (error: any) {
  ...
  const openEdit = (link: SharedLink) => {
    setEditing(link);
    setExpiry(dateInputValue(link.expiresAt));
  };
        ...
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Control who can access your shared content.
            </p>
          </div>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Set an optional expiry date. Clear it to keep the link available
              until you deactivate or delete it.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            {expiry && (
              <Button variant="outline" onClick={() => setExpiry("")}>
                Remove expiry
              </Button>
            )}
...
function ShareCard({
  ...
  return (
    <article
      className={`rounded-2xl border bg-card p-5 transition ${active ? "border-border" : "border-border/70 opacity-65"}`}
    >
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          ...
          <button
            onClick={() => onCopy(link)}
            className="mt-2 block max-w-full truncate text-left text-xs text-primary hover:underline"
          >
            {url}
          </button>
        ...
        <div className="flex shrink-0 items-center gap-1">
          <IconButton label="Copy link" onClick={() => onCopy(link)}>
            <Copy />
          </IconButton>
          <IconButton label={`${link.viewCount ?? 0} views`}>
            <BarChart3 />
          </IconButton>
          <IconButton label="Edit settings" onClick={() => onEdit(link)}>
            <Settings2 />
          </IconButton>
          <IconButton
            label={active ? "Deactivate link" : "Activate link"}
            onClick={() => onToggle(link)}
          >
            {active ? <ToggleRight className="text-primary" /> : <ToggleLeft />}
          </IconButton>
          <IconButton
            label="Delete link"
...
function Status({ active, expired }: { active: boolean; expired: boolean }) {
  return (
    <span
      className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${active ? "bg-green-100 text-green-700" : "bg-muted text-muted-foreground"}`}
    >
